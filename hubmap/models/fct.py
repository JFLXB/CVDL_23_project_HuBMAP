...
class Transformer(nn.Module):
    def __init__(
        ...
        self.attention_output = Convolutional_Attention(
...
class Block_decoder(nn.Module):
    def __init__(self, in_channels, out_channels, att_heads, dpr, img_size):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(
                out_channels * 2, out_channels, kernel_size=3, stride=1, padding="same"
            ),
            nn.ReLU(),
        )
...
class DS_out(nn.Module):
    def __init__(self, in_channels, out_channels, img_size):
        ...
        self.layernorm = nn.LayerNorm(
            [in_channels, img_size * 2, img_size * 2], eps=1e-5
        )
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                in_channels, in_channels, kernel_size=3, stride=1, padding="same"
            ),
            nn.ReLU(),
        )
...
class Block_encoder_with_skip(nn.Module):
    def __init__(self, in_channels, out_channels, att_heads, dpr, img_size):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(
                in_channels * 2, out_channels, kernel_size=3, stride=1, padding="same"
            ),
            nn.ReLU(),
        )
        # image size /= 2
...
class FCT(L.LightningModule):
    def __init__(self, args):
        ...
        self.block_1 = Block_encoder_without_skip(
            1, filters[0], att_heads[0], dpr[0], self.img_size
        )
        self.block_2 = Block_encoder_with_skip(
            filters[0], filters[1], att_heads[1], dpr[1], self.img_size // 2
        )
        self.block_3 = Block_encoder_with_skip(
            filters[1], filters[2], att_heads[2], dpr[2], self.img_size // 4
        )
        self.block_4 = Block_encoder_with_skip(
            filters[2], filters[3], att_heads[3], dpr[3], self.img_size // 8
        )
        self.block_5 = Block_encoder_without_skip(
            filters[3], filters[4], att_heads[4], dpr[4], self.img_size // 16
        )
        self.block_6 = Block_decoder(
            filters[4], filters[5], att_heads[5], dpr[5], self.img_size // 32
        )
        self.block_7 = Block_decoder(
            filters[5], filters[6], att_heads[6], dpr[6], self.img_size // 16
        )
        self.block_8 = Block_decoder(
            filters[6], filters[7], att_heads[7], dpr[7], self.img_size // 8
        )
        self.block_9 = Block_decoder(
            filters[7], filters[8], att_heads[8], dpr[8], self.img_size // 4
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (
            self.loss_fn(pred_y[2], y) * 0.57
            + self.loss_fn(pred_y[1], down1) * 0.29
            + self.loss_fn(pred_y[0], down2) * 0.14
        )
    ...
    def compute_dice(self, pred_y, y):
        ...
        for c in range(num_classes):
            ...
            dice_scores[c] = (2.0 * intersection + epsilon) / (sum_masks + epsilon)
