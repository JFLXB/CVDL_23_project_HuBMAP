...
class MultiOutputBCELoss(nn.Module):
    def __init__(self, weights: Tuple[float, float, float] = [1.0, 1.0, 1.0], interpolation_strategy: str = "bilinear"):
        super().__init__()
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        target1 = F.interpolate(target, size=input1.size()[2:], mode=self._interpolation_strategy)
        target2 = F.interpolate(target, size=input2.size()[2:], mode=self._interpolation_strategy)
